...
function stripClickableLinks(val: any): any {
	...
	if (val && typeof val === "object") {
		// biome-ignore lint/suspicious/noExplicitAny: generic output object
		const res: any = {};
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("before_agent_start", async (event, _ctx) => {
		const hint =
			"When referring to lines of code in specific files, always use the colon-notation format `path/to/file.ext:line` (e.g., `src/main.ts:15`)";
